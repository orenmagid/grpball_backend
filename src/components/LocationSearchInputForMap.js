...
export default class LocationSearchInputForMap extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      address: "",
  ...
  handleSelect = (address, placeId) => {
    geocodeByAddress(address)
      ...
      .then(latLng => {
        this.props.captureAddress(latLng);
      })
      .catch(error => console.error("Error", error));

    geocodeByAddress(address).then(results => {
      this.props.captureZoomType(results[0].types);
      this.setState({
          ...
          >
            {({
              ...
              <div>
                ...
                <div className="ui input bottom-margin">
                  <input
                    {...getInputProps({
                      placeholder:
                        this.props.type === "session"
